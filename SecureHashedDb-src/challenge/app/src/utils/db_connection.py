...
class db_Connection(object):
    ...
    def initMySql(self):
        return self.mysql.connect.cursor()
    ...
    def staticAdminSelector(self, cursor):
        cursor.execute(
            f"SELECT username FROM user as u JOIN roles as r ON u.id = r.id WHERE r.roleString = 'admin'"
        )
        got = cursor.fetchall()

        return (
            got[0]["username"] if len(got) > 0 else self.utils.generateRandomToken(20)
        )
